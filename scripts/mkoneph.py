...
def render(mode_str, reflections, smoothing_sigma=15):
    ...
    image = np.squeeze(interpolated)
    return gaussian(image, sigma=smoothing_sigma), qx, qy
...
def calculate(mode_str, reflections):
    ...
    image, qx, qy = render(mode_str, reflections=reflections, smoothing_sigma=15)
    np.save(OUTPUT / f"{mode_str}.npy", image)
