...
```{.matplotlib #fig:snse-z file="figures/snse/z.py" caption="Thermoelectric figure-of-merit where the effect of temperature has been factored out. This figure shows that the $Cmcm$ phase is an intrinsically-better thermoelectric than tin selenide in the $Pnma$."}
```
...
The $Cmcm$ phase is the better thermoelectric because the Seebeck coefficient $S$ does not significantly drop at higher temperatures, and that the thermal conductivity remains low[@Zhao2014;@Wei2019]; the temperature-dependence of these quantities is shown in @fig:snse-performance, reproduced from Zhao *et al.*[@Zhao2014]

```{.matplotlib #fig:snse-performance file="figures/snse/performance.py" caption="Determinants of thermoelectric performance in SnSe along crystallographic axes. **a)** Seebeck coefficient $S$. **b)** Lattice thermal conductivity $\kappa_l$."}
```
