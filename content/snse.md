...
```{.matplotlib #fig:snse-valley-scattering file="figures/snse/valley-scattering.py" caption="In-plane carrier relaxation process. Color intensity represents the photoelectron and photohole densities $N_{e^{-}}$ and $N_{h^{+}}$ respectively, where white is zero. The contours represent the dispersion of @fig:snse-electronic-structure. **a)** Carrier densities just after photoexcitation. **b)** Carrier densities after $\sim\SI{1}{\pico\second}$ showing that carriers have undergone intravalley scattering mediated by small wavevector phonons (arrows) to the local extrema of the conduction and valence bands. **c)** Carrier densities after $>\SI{5}{\pico\second}$ showing that carriers have undergone intervalley scattering mediated by large wavevector phonons (arrows) to the global extrema of the conduction and valence bands."}
```
