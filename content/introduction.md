...
In highly-ordered systems (i.e. crystals), where electronic correlation effects are most likely to matter, the time scales associated with fundamental actions are determined by the energy scale around room temperature. Phonons populated at room temperature (\SI{300}{\kelvin}) might have a period shorter than \SI{1}{\pico\second} (\SI{1e-12}{\second}), as $\omega = k_B T / \hbar = \SI{25}{\milli\electronvolt} / \hbar = \SI{6}{\tera\hertz}$. 
...
![Effect of mistiming of electron compression on the electron bunch. The zero-crossing determines the moment where the electric field is 0 in the cavity. If the compression zero-crossing happens too early, electron bunches are effectively slowed down, and arrive late at the sample relative to the photoexcitation. On the other hand, if the compression zero-crossing happens too late, electron bunches are accelerated, and arrive early at the sample relative to the laser excitation.](diagrams/rf-timing.pdf){#fig:intro-zero-crossing}
...
```{.matplotlib #fig:intro-sync-spectrum file="figures/introduction/freq-comb.py" caption="Spectrum of a \SI{75}{\mega\hertz} oscillator pulse train on a high-bandwidth (\SI{12.5}{\giga\hertz}) photodiode, with an input power of \SI{20}{\milli\watt}. **a)** Full bandwidth measurement shows harmonics up to at least \SI{6}{\giga\hertz}. **b)** Spectrum centered on the 40^th^ harmonic of the oscillator repetition rate."}
```
